...
public class FruitsMain {
	...
	public static void main(String[] args) {
		...
		List<Fruits> Fruit_list = new LinkedList<>();
        Fruit_list.add(new Fruits("Apple", 17, 234, "red"));
        Fruit_list.add(new Fruits("Banana", 15, 230, "yellow"));
        Fruit_list.add(new Fruits("mango", 150, 254, "Yellow"));
        Fruit_list.add(new Fruits("orange", 120, 158, "orange"));
        //1. Fruits with calories less than 100
        System.out.println("Fruits with calories less than 100");
        Fruit_list.stream().filter(x -> x.getCalories() < 100).map(Fruits::getName).forEach(System.out::println);
        //2. Sorting fruits according to their color
        System.out.println("Sorting fruits according to their color");
        Fruit_list.stream().sorted(Comparator.comparing(Fruits::getColor)).map(Fruits::getName).forEach(System.out::println);
